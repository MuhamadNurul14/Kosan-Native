<?php
// File: views/categories.php

// Ambil slug kategori dari URL (misal: /categories/kost-putra)
$category_slug = $id ?? 'all';
...
$category_display_name = "All Categories";
if ($category_slug !== 'all' && !empty($filtered_kos)) {
    $category_display_name = $filtered_kos[0]['category_name'];
}
?>
...
<div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
    <div class="flex flex-col gap-[6px]">
        <h1 class="font-bold text-[32px] leading-[48px]"><?php echo htmlspecialchars($category_display_name); ?></h1>
...
<section id="Result" class="relative flex flex-col gap-4 px-5 mt-5 mb-9">
    <?php if (empty($filtered_kos)): ?>
        <p class="text-center text-ngekos-grey py-10">Tidak ada kos yang tersedia dalam kategori ini.</p>
    <?php else: ?>
